...
fun <T : Any?> MutableLiveData<Event<T>>.postValueEvent(newValue: T) =
    apply { postValue(Event(newValue)) }

fun <T : Any?> MutableLiveData<Event<T>>.setValueEvent(newValue: T) =
    apply { value = Event(newValue) }